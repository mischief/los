...
#define iodelay() \
	__asm__("pushl %eax; inb $0x80,%al; inb $0x80,%al; popl %eax")

#define inl(port) \
({ unsigned long _tmp__; \
	__asm__ __volatile__("inl %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ unsigned short _tmp__; \
	__asm__ __volatile__(".byte 0x66; inl %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ unsigned char _tmp__; \
	__asm__ __volatile__("inb %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ __asm__ __volatile__("outl %0, %%dx" : : "a" (val) , "d" ((unsigned short)(port))); })
...
({__asm__ __volatile__(".byte 0x66; outl %0, %%dx" : : "a" ((unsigned short)(val)) , "d" ((unsigned short)(port))); })
...
({ __asm__ __volatile__("outb %0, %%dx" : : "a" ((unsigned char)(val)) , "d" ((unsigned short)(port))); })
