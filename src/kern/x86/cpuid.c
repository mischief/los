...
static void get_cache_config(struct cpu_info *id)
{
  ...
  do {
    ...
    cicount++;
    __asm__ __volatile__("cpuid" : "=a" (ci[0]), 
      "=b" (ci[1]), "=c" (ci[2]), "=d" (ci[3]) : "a" (2));
	...
	if (i == 0)
	  reg >>= 8;
	while (reg != 0) {
	  if ((reg & 0xff) && (ccidx < sizeof(id->cache_config))) {
	    id->cache_config[ccidx++] = reg & 0xff;
	  }
	  reg >>= 8;
	}
      }
...
void cpuid(struct cpu_info *out_id)
{
  ...
  if ((get_eflags() ^ orig_eflags) & EFL_AC) {
	 ...
	 * First get the vendor ID string. */
      __asm__ __volatile__("cpuid" : "=a" (highest_val), "=b" (*((int*)(out_id->vendor_id+0))),
	"=d" (*((int*)(out_id->vendor_id+4))), "=c" (*((int*)(out_id->vendor_id+8))) : "a" (0));

      /* Now the feature information.  */
      if (highest_val >= 1) {
	__asm__ __volatile__("cpuid" : "=a" (*((int*)out_id)),
	  "=d" (out_id->feature_flags) : "a" (1) : "ebx", "ecx");
      }
